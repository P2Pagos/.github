...
P2Payments объединяет несколько входных rails — локальный фиат, карты, P2P и криптовалюту — с on-chain settlement в Bitcoin, USDT в Polygon или других стейблкоинах в Liquid.
...
P2Payments построена вокруг нескольких практических принципов:
...
- **Open source** — публичные компоненты остаются под лицензией MIT

Если какой-либо rail не производит settlement в активе, уже поддерживаемом форком Aqua Wallet, P2Payments стремится дополнительно конвертировать средства в поддерживаемый актив, который будет самым дешёвым и наиболее функциональным для данного случая.
...
MIT-форк Aqua Flutter Wallet для P2Payments со встроенным приложением Nuxt для управления настройками /mono и подключения к BTCPay через протокол Shamrock.
...
P2Payments ориентирована на случаи, когда стандартные платёжные стеки слишком ограничены, слишком хрупки или слишком сильно зависят от одного провайдера.
...
P2Payments всё ещё развивается. Некоторые компоненты уже существуют как рабочие интеграции, другие частично реализованы, экспериментальны или ещё собираются в основной orchestrator.
